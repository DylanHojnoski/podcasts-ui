...
export class FeedService {
  ...
  const headers = new HttpHeaders().set('Accept', 'application/xml');
    return this.http.get(`${environment.apiUrl}/${this.url}/opml`, { headers, responseType: 'text' , withCredentials: true });
  }

  public importOPML(data: FormData) : Observable<any[]> {
    return this.http.post<any[]>(`${environment.apiUrl}/${this.url}/opml`, data ,{ withCredentials: true });
  }
